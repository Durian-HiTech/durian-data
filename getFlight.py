    ...
    for src in code[begin_pos[0]: len(city_list)]:
        ...
        for dst in code[begin_pos[1]: len(city_list)]:
            ...
            if begin_pos[0] == 0 and begin_pos[1] == 0:
                # with open(f'./flights_data/flights_data{now}.json', 'r+', encoding='utf-8') as fp:
                #     context_now = fp.read()
                #     if context_now == '':
                #         context_now = []
                #     else:
                #         context_now = json.loads(context_now)
                with open(f'./flights_data/flights_data{now}.json', 'w+', encoding='utf-8') as fp:
                    # context_now.extend(results)
                    fp.write(json.dumps(results, ensure_ascii=False))
            else:
                with open(f'./flights_data/flights_data{args.date}.json', 'r+', encoding='utf-8') as fp:
                    context_now = fp.read()
                    if context_now == '':
                        context_now = []
                    else:
                        context_now = json.loads(context_now)
                with open(f'./flights_data/flights_data{args.date}.json', 'w+', encoding='utf-8') as fp:
                    context_now.extend(results)
...
def main():
    ...
    if args.date:
        try:
            with open(f'./flights_data/flights_data{args.date}.json', 'r+', encoding='utf-8') as f:
        ...
        try:
            with open(f'flights_data/flights_data{datetime.now().strftime("%Y-%m-%d")}.json', 'r+', encoding='utf-8') as f:
                context = json.loads(f.read())[-1]
                begin_pos[0] = code.index(context['dept_city'])
                begin_pos[1] = code.index(context['arri_city'])
